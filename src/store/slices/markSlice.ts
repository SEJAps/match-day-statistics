...
const markSlice = createSlice({
  ...
  reducers: {
    ...
    removeMark: (
      state,
      action: PayloadAction<{ team: string; time_stamp: number }>,
    ) => {
      const { team, time_stamp } = action.payload;
      if (team === "local") {
        state.local = state.local.filter(
          (mark) => mark.time_stamp !== time_stamp,
        );
      } else {
        state.guest = state.guest.filter(
          (mark) => mark.time_stamp !== time_stamp,
        );
      }
    },
    updateMark: (state, action: PayloadAction<Mark>) => {
      ...
      if (team === "local") {
        state.local = state.local.map((mark) =>
          mark.time_stamp === data.time_stamp ? data : mark,
        );
      } else {
        state.guest = state.guest.map((mark) =>
          mark.time_stamp === data.time_stamp ? data : mark,
        );
      }
    },
