...
const statsSlice = createSlice({
  ...
  reducers: {
    ...
    updateTeamName: (
      state,
      action: PayloadAction<{ team: TTeam; name: string }>,
    ) => {
      state[action.payload.team === "local" ? "localName" : "guestName"] =
        action.payload.name;
    },
    saveStats: (state) => {
      createStatsByMatchActive({
...
export const { resetStats, updateTeamName, increment, decrement, saveStats } =
  statsSlice.actions;
