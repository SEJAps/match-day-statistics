...
const ImageWebp: FC<TImage> = ({ src, alt, onClick, title, image_down }) => {
  return (
    <img
      onClick={onClick}
      className={`${image_down ? webpCSS.image_down : webpCSS.image} ${webpCSS.icon}`}
      src={src}
      alt={alt}
      width={32}
      height={32}
...
const ImageStat: FC<TImage> = ({ src, alt, onClick, title, image_down }) => {
  return (
    <img
      onClick={onClick}
      className={`${image_down ? webpCSS.image_down : webpCSS.imageStat} ${webpCSS.icon}`}
      src={src}
      alt={alt}
      width={32}
      height={32}
...
const CheckRojo: FC<{ onClick: () => void; image_down?: boolean }> = ({
  onClick,
  image_down,
}) => {
  return (
...
const CheckVerde: FC<{ onClick: () => void; image_down?: boolean }> = ({
  onClick,
  image_down,
}) => {
  return (
...
const IconStat: FC<{ onClick: () => void; title?: string; stat: string }> = ({
  onClick,
  title,
  stat,
}) => {
  ...
  return (
    <button className={webpCSS.btn}>
      <ImageWebp
        onClick={onClick}
        src={
          statIcons.find((record) => record.stat === stat.toLowerCase())
            ?.src as string
        }
        alt={stat}
        title={title}
...
const IconMark: FC<{ onClick: () => void; title?: string; stat: string }> = ({
  onClick,
  title,
  stat,
}) => {
...
export { CheckRojo, CheckVerde, IconStat, IconMark };
