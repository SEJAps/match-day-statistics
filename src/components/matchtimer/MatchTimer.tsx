      ...
      {half !== "finished" ? (
        <>
          ...
          <div className={timeCSS.timer_display}>
            ...
            {extraTime > 0 && <span className="extra-time">+{formatTime(extraTime)}</span>}
          </div>
          ...
          <div className={timeCSS.controls}>
            <button className={timeCSS.btn_pause} onClick={() => dispatch(pause())}>
              {isRunning ? t("pause") : t("resume")}
            </button>
        
            <button className={timeCSS.btn_reset} onClick={() => dispatch(reset())}>
              {t("reset_timer")}
            </button>
          </div>
        </>
        ...
        <div className={timeCSS.results}>
          ...
          <p>1ª {t("part")}: {formatTime(firstHalfTime)} +{formatTime(firstHalfExtra)}</p>
          <p>2ª {t("part")}: {formatTime(secondHalfTime)} +{formatTime(secondHalfExtra)}</p>
          <button className={timeCSS.btnReset} onClick={() => dispatch(reset())}>
            {t("reboot")}
          </button>
        </div>
      )}
